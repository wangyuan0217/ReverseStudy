...
def parse_FMT35C(buffer, dex_object, pc_point, offset):
    ...
    if A == 5:
        ...
        return (
            dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % C, "v%d" % D, "v%d" % E,
            "%s" % (prefix))
    elif A == 2:
...
def parseinsfile():
    ...
    insfile.close()
    # {"name":"void Decoder.BASE64Decoder.<init>()","method_idx":1,"offset":1850820,"code_item_len":34,"ins":"AgABAAEAAAAaahgACQAAAHAQDwABABJAIwBFEFsQAAAOAA=="};
    insarray = content.split("};")
    for eachins in insarray:
        eachins = eachins + "}"
        try:
            eachins_json = json.loads(eachins)
            methodname = eachins_json["name"]
            number = eachins_json["method_idx"]
            offset = eachins_json["offset"]
            inssize = eachins_json["code_item_len"]
            ins = eachins_json["ins"]
        ...
        if self.tries_size > 0:
            for i in range(0, self.tries_size):
                ...
                for j in range(len(tryitem.handlerlist)):
                    ...
                    print "%s%-20s:%08x:%10d" % (
                        prefix, "try[" + str(i) + "]:handler[" + str(j) + "] ins start:", temphandler.ins_start,
                        temphandler.ins_start)
        ...
        for i in xrange(0, self.numStaticFields):
            ...
            if self.staticValuesOff:
                ...
                if staticoffset == -1:
                    str1 += "0;\n"
                    continue
                size, str2 = parse_encoded_value1(dex_object,
                                                  dex_object.m_content[self.staticValuesOff + staticoffset:])
                str1 += str2
            str1 += ";\n"
        field_idx = 0
        str1 += "////////////////////////////////////////////////////////\n"
...
def init():
    print 'fart_repire.py -d <dumpdexfile> -i <insfile> >> repired.txt'
    global dexfilename
    global insfilename
    try:
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print 'fart_repire.py -d <dumpdexfile> -i <insfile>'
            sys.exit()
